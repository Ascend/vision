...
class TestNpuAdjustSaturation(TestCase):
    @staticmethod
    def cpu_op_exec(input1, factor):
        output = torchvision.transforms.functional.adjust_saturation(input1, factor)
        output = output.numpy()
        return output

    @staticmethod
    def npu_op_exec(input1, factor):
        output = torch.ops.torchvision.npu_adjust_saturation(input1, factor)
        output = output.cpu().squeeze(0)
        output = output.numpy()
        return output

    def test_npu_adjust_saturation(self):
